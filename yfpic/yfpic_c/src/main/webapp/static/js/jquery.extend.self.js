...
$.extend({
  getImgIds:function(){
    var aPicId=[];
	var oLis = $('.imgUl').find('li');
	for (var i = 0; i < oLis.length; i++) {
          for (var j = oLis.length; j > 0; j--) {
              if (oLis[j]!=undefined && (oLis[j].offsetTop < oLis[j-1].offsetTop)) {
            	 var temp = oLis[j-1];
  				 oLis[j-1] = oLis[j];
  				 oLis[j] = temp; 
              }else if(oLis[j]!=undefined && (oLis[j].offsetTop==oLis[j-1].offsetTop)){
				if(oLis[j].offsetLeft<oLis[j-1].offsetLeft){
					var temp = oLis[j-1];
					oLis[j-1] = oLis[j];
					oLis[j] = temp; 
				}
	...
	for(var b =0;b<oLis.length;b++){
		 aPicId.push($(oLis[b]).children()[0].getAttribute('data-picId'));
	} 
	return aPicId;
  },
  getImgSrcs:function(){
	  ...
	  for(var b =0 ;b<oLis.length;b++){
		  aImgSrc.push($(oLis[b]).children()[0].src);
	  } 
	  return aImgSrc;
  },
  imgDrag: function(obj){
		...
		for(var i=0;i<obj.aImg.length;i++){
			aLiHtml.push('<li style="width:'+iWidth+'px;height:'+iHight+'px"><img src="'+obj.aImg[i]+'" data-picId="'+aPic[i]+'"/>'+
					'<span class="quickdelete"></span></li>');
		}
